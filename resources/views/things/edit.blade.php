...
<form action="{{ route('things.update', $thing) }}" method="POST">
    ...
    <div class="mb-3">
        ...
        <textarea class="form-control" id="description" name="description" rows="4">{{ old('description', $thing->description) }}</textarea>
    </div>
    <div class="mb-3">
        <label for="wrnt" class="form-label">Гарантия/Срок годности</label>
        <input type="date" class="form-control" id="wrnt" name="wrnt" value="{{ old('wrnt', $thing->wrnt ? $thing->wrnt->format('Y-m-d') : '') }}">
    </div>
